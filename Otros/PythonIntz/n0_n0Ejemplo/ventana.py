import tkinter as tk
from tkinter import ttk
from screens.elementos import elementos

class ventana(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Curso Tkinter")
        self.center_window(self,300,500)
        self.resizable(False,False)
        
        #thema
        self.forest=ttk.Style(self)
        self.tk.call("source",r"theme\\ForestLight\\forest-light.tcl")
        self.forest.theme_use("forest-light")

        #contenido
        frame = elementos(self)
        frame.pack(
            side=tk.TOP,
            fill=tk.BOTH,
            expand=True
        )
         
    def center_window(self,window,alto,ancho):
        x = (window.winfo_screenwidth() // 2) - (ancho // 2)
        y = (window.winfo_screenheight() // 2) - (alto // 2)

        window.geometry(f'{ancho}x{alto}+{x}+{y}')

